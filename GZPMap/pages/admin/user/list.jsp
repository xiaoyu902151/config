...
<body style="overflow:hidden;">
    <div class="top_001">
        ...
        <span style="height: 35px; width: 350px;">当前位置：后台管理&nbsp;>&nbsp;<label >用户管理</label></span>
    </div>
    ...
    <div id="queryPanel">
    	<div class="form form-inline" role="form">
    		<div class="form-group" style="margin:10px 10px;">
        		...
        		<input class="form-control" id="loginName" type="text" placeholder="登录账号" >
			</div>
			<div class="form-group" style="margin:10px 10px;">
	        	<label class="control-label" >用户姓名： </label>
	        	<input class="form-control" id="realName" type="text" placeholder="用户姓名" >
        	</div>
			<div class="form-group" style="margin:10px 10px;">
	...
	<div id="toolbar" style="padding:0px 0px 5px 5px;">
        ...
        <button id="remove" class="btn btn-danger" onclick="roleEdit()">
            <i class="glyphicon glyphicon-remove"></i> 角色授权
        </button>
        <button id="resetPassWork" class="btn btn-danger" onclick="resetPassWork()">
            <i class="glyphicon glyphicon glyphicon-refresh"></i> 重置密码
        </button>
    </div>
...
<script>
    ...
    $(function () {
			...
			datagrid = datagrid.bootstrapTable({
                url: "${pageContext.request.contextPath}/admin/user.do?searchGrid",//数据源
 ...
 function resetPassWork()
		{
		  var idName = "userId";
			...
			if (selRows.length != 1 && selRows.length != 0) {
				...
				//弹出编辑详情框
				 objDialog = jDialog.iframe("${pageContext.request.contextPath}/pages/admin/user/resetView.jsp?userId="+selRows[0][idName],{	                
		                title: '密码重置',
		                width: 700,
		                height:350,
		                modal: true, // 非模态，即不显示遮罩层
		    ...
		    if(selRows.length > 0) {
		    	var dialog = jDialog.confirm("您要删除当前所选项目？",{
					handler : function(button,dialog) {
						...
						$.ajax({
							url : "${pageContext.request.contextPath}/admin/user.do?delete",
							data : {ids : ids.join(',')},
							cache : false,
							dataType : "json",
