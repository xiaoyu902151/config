...
<body style="overflow:hidden;">
	 <div class="top_001">
        ...
        <span style="height: 35px; width: 350px;">当前位置：应急预案&nbsp;>&nbsp;<label>应急知识库</label></span>
    </div>
    ...
    <div id="queryPanel">
      <div class="form form-inline">
    	<div class="form-group" style="margin:10px 10px;">
	        <label class="control-label" >危险品名称： </label>
	        <input class="form-control" id="chName" type="text" placeholder="危险品名称" >
	    </div>
	    <div class="form-group" style="margin:10px 10px;">
	        <label class="control-label" >英文名： </label>
	        <input class="form-control" id="enName" type="text" placeholder="英文名" >
	    </div>
	    <div class="form-group" style="margin:10px 10px;">
	        <label class="control-label" >危险品编号： </label>
	        <input class="form-control" id="hwbh" type="text" placeholder="危险品编号" >
       	</div>
...
<script>
    ...
    $(function () {
			...
			datagrid = datagrid.bootstrapTable({
                url: "${pageContext.request.contextPath}/support/degKnowlage.do?searchGrid",//数据源
       ...
       function infoFormatter(value, row, index)
        ...
        function add()
		{
		        objDialog = jDialog.iframe("${pageContext.request.contextPath}/support/degKnowlage.do?addView",{	                title: '新增危险品',
		                width: 800,
		                height:700,
		    ...
		    if(selRows.length > 0) {
		    	var dialog = jDialog.confirm("您要删除当前所选项目？",{
					handler : function(button,dialog) {
						...
						$.ajax({
							url : "${pageContext.request.contextPath}/support/degKnowlage.do?delete",
							data : {ids : ids.join(',')},
							cache : false,
							dataType : "json",
		...
		function modify()
		{
			...
			if (selRows.length != 1 && selRows.length != 0) {
				...
				//弹出编辑详情框
				objDialog =  jDialog.iframe("${pageContext.request.contextPath}/support/degKnowlage.do?updateView&id="+selRows[0][idName],{
					title : '修改',
					width : 800,
					height : 700,
					iframeId : "jDialogFrame"///对话框iframe的ID
		...
		function browse()
		{
			...
			if (selRows.length != 1 && selRows.length != 0) {
				...
				//弹出编辑详情框
				objDialog =  jDialog.iframe("${pageContext.request.contextPath}/support/degKnowlage.do?browseView&id="+selRows[0][idName],{
					title : '查看知识库',
					width : 800,
					height : 700,
					iframeId : "jDialogFrame"///对话框iframe的ID
		...
		function editData(frmName,formName,AjaxUrl,fileRows,delRows) {
		     var objForm = $("#"+frmName).contents().find("#"+formName);
		     var arry = objForm.serializeArray();
		     var parm = {afs:fileRows,dfs:delRows};
		...
		function addData(frmName,formName,AjaxUrl,fileRows) {
		    var objForm = $("#"+frmName).contents().find("#"+formName);
		    var arry = objForm.serializeArray();
